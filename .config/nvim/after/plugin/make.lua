vim.keymap.set("n", "<Leader>ma", function()
	...
	if not vim.uv.fs_stat(fname) then return end
	...
	local query = vim.treesitter.query.parse("make", [[
		(rule
			(targets) @__target (#not-eq? @__target ".PHONY")
		) @__rule
	]])
	...
	require("tigh-latte.pickers.fzf").bottom({
		...
		callback = function(item)
			vim.system({ "make", item }, {}, function(out) print(out.stdout, "\n", out.stderr) end)
		end,
		title = "make",
		height = 0.2,
		preview = function(item) return rules[item] end,
	})
end, {})
